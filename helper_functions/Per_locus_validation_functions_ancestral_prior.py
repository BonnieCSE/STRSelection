...
def validate_per_locus(per, opt_allele, s_vals, use_het, use_common, use_bins, \
                       ...
                       first_200=True, trials_2000=False, LRT_200=False, eps_bins=0.3):
    
    
    # Uniform prior
    if per == 2:
        opt_allele_for_per = list(range(11,21))
    if per == 3:
        opt_allele_for_per = list(range(5,14))
    if per == 4:
        opt_allele_for_per = list(range(7,11))
        
    # Get dictionary of abc_lists
    dict_abc_lists = {}
    for optimal_allele in opt_allele_for_per:
        abcFile = '/gymreklab-tscc/bonnieh/abc/results/' + abc_model +'/' + str(per) + '_' + str(optimal_allele) + '.txt' 

        # Read abcFile line by line and place in lookup table in the form of a list
        abc_list = GetABCList(abcFile, num_bins)
        dict_abc_lists[optimal_allele] = abc_list
        
    # Get LRT summary statistic tables for s = 0
    dict_LRT_table_0_het = {}
    dict_LRT_table_0_common = {}
    dict_LRT_table_0_bins = {}
    
    for optimal_allele in opt_allele_for_per:
        lrtFile_for_s_0 = '/gymreklab-tscc/bonnieh/lrt/results/' + lrt_model + '/' + str(per) + '_' + str(optimal_allele) + '_13_freqs.txt' 
        ...
        dict_LRT_table_0_het[optimal_allele] = LRT_table_0_het
        dict_LRT_table_0_common[optimal_allele] = LRT_table_0_common
        dict_LRT_table_0_bins[optimal_allele] = LRT_table_0_bins
    ...
    for opt_allele in opt_allele_list:
        ...
        obs_modal_allele_dic = {} # Key - s; value - list of modal alleles
        ...
        for s in s_vals:
            ...
            for freq_string in freqs_list_raw:
                ...
                freq_string = freq_string.split(',')
                freq_string = [float(i) for i in freq_string]
                max_value = max(freq_string)
                modal_allele = freq_string.index(max_value)
                modal_allele = modal_allele - 12
                obs_modal_allele_dic[s].append(modal_allele)
        ...
        for s in s_vals:
            ...
            list_common = []
            list_bins = []
            list_ancestral_allele = []
            
            # Lists to keep track of upper and lower bound of confidence intervals from ABC
            list_ci_lower_bound = []
            list_ci_upper_bound = []
            ...
            for i in range(0, len(obs_het_dic[s])):
                ...
                obs_modal_allele = obs_modal_allele_dic[s][i]
                
                # Generate abc_list
                abc_list = []
                
                prior_ancestral_allele = list(range(obs_modal_allele + opt_allele, obs_modal_allele + opt_allele + 5))
                #print(prior_ancestral_allele)
                list_random_ancestral_alleles = []
                for k in range(0, 10000):
                    random_num = random.choice(prior_ancestral_allele)
                    if random_num in opt_allele_for_per:
                        abc_list.append(random.choice(dict_abc_lists[random_num]))
                        list_random_ancestral_alleles.append(random_num)
                
                s_ABC, lower_bound, upper_bound, num_accepted, s_accepted, list_pos_accepted = Get_S_ABC(abc_list, \
                                       obs_het, obs_common, obs_bins, constant_het, \
                                       denom_het, constant_common, denom_common, eps_bins, use_het, \
                                       use_common, use_bins, return_positions=True)
                
                list_accepted_alleles = []
                
                for pos in list_pos_accepted:
                    allele = list_random_ancestral_alleles[pos]
                    list_accepted_alleles.append(allele)
                    
                #print(np.mean(list_accepted_alleles))
                ...
                if s_ABC != -1:
                    ...
                    list_ancestral_allele.append(np.median(list_accepted_alleles))#(stats.mode(list_accepted_alleles).mode[0])
                    
                    if lower_bound < 10**-6:
                        lower_bound = 10**-6
                    list_ci_lower_bound.append(lower_bound)
                    list_ci_upper_bound.append(upper_bound)
            ...
            obs_ancestral_allele_dic_lrt[s] = list_ancestral_allele
            
            # Put mean of esimated s in s_vals_dic and calculate standard deviation
            s_vals_dic[opt_allele].append(np.mean(list_est_s)) 
            
            # Put standard deviation as error
            #std_err = np.std(list_est_s)
            #errors_s_dic[opt_allele].append(std_err)
            
            # Put mean of upper and lower bound of CI as error
            errors_s_dic[opt_allele].append((np.mean(list_ci_lower_bound), np.mean(list_ci_upper_bound)))
            
        print('No ABC acceptances')
        print(no_ABC_accept)
        ...
        for s in est_s_dic:
            ...
            obs_ancestral_allele_list = obs_ancestral_allele_dic_lrt[s]
            ...
            for i in range(0, len(obs_het_list)):
                ...
                obs_ancestral_allele = obs_ancestral_allele_list[i]
                ...
                if s_ABC_round not in s_list_available:
                    s_ABC_round = getNearestS(s_ABC_round, s_list_available)
                '''
                lrtFile = '/gymreklab-tscc/bonnieh/lrt/results/' + lrt_model + '/' + str(per) + '_' + str(obs_ancestral_allele) + '_freqs.txt'
                lrtFile_for_s_0 = '/gymreklab-tscc/bonnieh/lrt/results/' + lrt_model + '/' + str(per) + '_' + str(obs_ancestral_allele) + '_13_freqs.txt' 
                ...
                LRT_table_0_het = dict_LRT_table_0_het[obs_ancestral_allele]
                LRT_table_0_common = dict_LRT_table_0_common[obs_ancestral_allele]
                LRT_table_0_bins = dict_LRT_table_0_bins[obs_ancestral_allele]
